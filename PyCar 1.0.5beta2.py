...
def save2file(hscore):
    now = datetime.datetime.now()
    savedate= str(now.day)+ '-'+ str(now.month)+ '-'+str(now.year)+ ' | '+ str(now.hour)+':'+ str(now.minute)+':'+str(now.second)
    savescore = '\n# |  ' +'  Date: ' + savedate
    print(savescore)
    saveFile = open('Save\HiScores.txt', 'a')
    saveFile.write(savescore)
    saveFile.close()
...
def paused():
    ...
    while pause:
        ...
        button('Vroom again!', 150, 450, 150, 50, green, bright_green,unpause)
